...
async function callImageToVideo(payload) {
  if (typeof base44?.functions?.generateImageToVideo === "function") {
    return base44.functions.generateImageToVideo(payload);
  }
  if (typeof base44?.functions?.invoke === "function") {
    return base44.functions.invoke("generateImageToVideo", payload);
  }
  if (typeof base44?.integrations?.Core?.InvokeFunction === "function") {
    return base44.integrations.Core.InvokeFunction({ name: "generateImageToVideo", data: payload });
  }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      newRecord = await base44.entities.GeneratedVideo.create({
        ...
        generation_payload_debug: JSON.stringify({ route, owner_user_id: ownerFields.owner_user_id, owner_email: ownerFields.owner_email }),
        likes: 0,
      });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-5">
            ...
            <div>
              ...
              <Textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Describe the video you want to generate..." className="min-h-[120px] text-sm resize-none" />
            </div>
          ...
          <div>
            <div className="border border-border rounded-xl overflow-hidden bg-card min-h-[500px] flex flex-col">
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6">
                ...
                {!isGenerating && !generatedItem && <div className="text-center text-muted-foreground"><Video className="w-12 h-12 mx-auto mb-3 opacity-30" /><p className="text-sm">Your generated video will appear here</p><p className="text-xs mt-1 opacity-70">Only your videos will appear in your gallery.</p></div>}
                {generatedItem?.video_url && !isGenerating && <div className="w-full"><div className="rounded-lg overflow-hidden border border-border mb-4 bg-black"><video src={generatedItem.video_url} controls autoPlay loop className="w-full object-contain max-h-80" poster={generatedItem.thumbnail_url || generatedItem.reference_image_url} /></div><div className="bg-muted/30 rounded-lg p-3 border border-border"><p className="text-xs font-medium text-muted-foreground mb-1">Prompt</p><p className="text-sm text-foreground leading-relaxed">{generatedItem.prompt}</p></div><div className="mt-4 flex gap-2"><Link to="/gallery" className="flex-1"><Button variant="outline" className="w-full text-sm">View in Gallery</Button></Link></div></div>}
              </div>
            </div>
          </div>
        </div>

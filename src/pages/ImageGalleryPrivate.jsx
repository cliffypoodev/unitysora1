...
export default function ImageGalleryPrivate() {
  ...
  const allFilteredSelected = filtered.length > 0 && filtered.every((image) => selectedImageIds.has(image.id));
  ...
  const handleSelectAll = () => {
    setSelectedImageIds(allFilteredSelected ? new Set() : new Set(filtered.map((image) => image.id)));
  };

  const handleDeleteSelected = async () => {
    if (selectedCount === 0) return;
    const confirmed = window.confirm(`Delete ${selectedCount} selected image${selectedCount === 1 ? "" : "s"}? This cannot be undone.`);
    if (!confirmed) return;

    const idsToDelete = Array.from(selectedImageIds);
    await Promise.all(idsToDelete.map((id) => base44.entities.GeneratedImage.delete(id)));
    setImages((prev) => prev.filter((image) => !selectedImageIds.has(image.id)));
    if (selectedImage && selectedImageIds.has(selectedImage.id)) setSelectedImage(null);
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {selectedCount > 0 && (
              <>
                ...
                <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedImageIds(new Set())} className="gap-2">
          ...
          <div className="columns-2 sm:columns-3 md:columns-4 gap-4 space-y-4">
            {filtered.map((image, index) => (
              <motion.div
                ...
                className={`break-inside-avoid rounded-xl overflow-hidden border border-border bg-card group shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer mb-4 ${selectedImageIds.has(image.id) ? "ring-2 ring-destructive" : ""}`}
              >
                <div className="relative overflow-hidden bg-black">
                  <button onClick={(event) => toggleImageSelection(event, image.id)} className="absolute top-2 left-2 z-10 flex items-center justify-center bg-black/50 hover:bg-black/75 backdrop-blur-sm text-white rounded-full p-1.5 transition-all">
                    {selectedImageIds.has(image.id) ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
                  </button>
